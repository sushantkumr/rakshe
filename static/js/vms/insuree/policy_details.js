$(document).ready(function() {
    ...
    function PolicyDetailsViewModel () {
        ...
        self.submit = function(retry_attempt) {
            ...
            ec.utils.ajax('policy', 'views', 'check_fee_paid', data, function(response) {
                if (!response.success) {
                    if (self.feeTx() && response.message === 'The tx with corresponding hash could not be found. Please verify the hash or try again after sometime.') {
                        if (retry_attempt < 2) {
                            bootbox.hideAll();
        ...
        self.init = function() {
            ...
            ec.utils.ajax('policy', 'views', 'get_policy_details', {id: policyId}, function(response) {
                if (!response.success) {
