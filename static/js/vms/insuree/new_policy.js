$(document).ready(function() {
    ...
    function NewPolicyViewModel () {
        var self = this;

        self.liveStatus = ko.observable(Globals.liveStatus);
        self.error = ko.observable();

        self.importantNote = ko.observable(true);

        self.approvedDapps = ko.observableArray();
        ec.utils.ajax('policy', 'views', 'get_approved_dapps', {}, function(response) {
            if (!response.success) {
                ec.utils.errorHandler(response, self.error);
                return;
            }
            self.approvedDapps(response.data);
        });
        ...
        self.contractDetailsReady = ko.computed(function() {
            ...
            if (self.chosenDapp() === undefined) {
                ...
                return ec.utils.ajax('policy', 'views', 'get_contract_info', {dapp_id: self.chosenDapp().id}, function(response) {
                    if (!response.success) {
                        ec.utils.errorHandler(response, self.error);
                        return false;
                    }
        ...
        self.message = ko.computed(function() {
            ...
            var padding = ' *************************************************** ';
            var messageHead = [
                'I agree to the terms and conditions mentioned in the New Warranty page on Rakshe.com. ',
            ...
            var coverageLimitMessage = ' Coverage Limit: ' + String(Number(self.coverageLimit())) + ' ETH.';
            var feeMessage = ' Fee: ' + self.fee() + ' ETH';
            var signedByInsuree = ' Signed by address (warranty buyer): ' + self.web3.accounts()[0];
            ...
            if (self.liveStatus() === 1) {
                var testSiteNotice = 'WARNING: THIS IS A TEST SITE FOR DEMONSTRATION ONLY. REAL ETH WILL NOT BE COLLECTED AS FEE AND A REAL WARRANTY WILL NOT BE ISSUED.';
                message = [
                    testSiteNotice,
                    padding,
                    message,
        ...
        self.submitApplication = function() {
            ...
            var address = self.web3.web3.personal.ecRecover(self.message(), self.signature(), function(err, address) {
                ...
                ec.utils.ajax('policy', 'views', 'submit_policy_application', data, function(response) {
                    if (!response.success) {
                    ...
                    ec.utils.bootboxInformation('Your application has been submitted.', function() {
                        window.location = '/insuree/policy_details?id=' + response.data.id;
                    });
                });
            });
        };
    ...
    _viewModel = new NewPolicyViewModel();
    ko.applyBindings(_viewModel);
    $('#js--insuree-new_policy').addClass('active');
});
